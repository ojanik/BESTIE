...
class Pipeline():
    ...
    def _set_optimization_pipeline(self):
        @partial(jit,static_argnames=["training"])
        def optimization_pipeline(net_params,data,weights,grad_weights,sample_weights,training=False,drop_out_key=None):
    ...
    def get_hist(self,lss,weights,sample_weights=None):

        if sample_weights is not None:
                sample_weights = jnp.reshape(sample_weights,weights.shape)
                weights = weights * sample_weights

        counts = self.calc_hist(lss, weights=weights)
