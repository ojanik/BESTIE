...
def create_cosine_reset_lr(base_learning_rate,steps_per_epoch=1,num_epochs=1000,warmup_epochs=10, number_of_resets = 3):
    warmup_fn = optax.linear_schedule(
      init_value=0., end_value=base_learning_rate,
      transition_steps=warmup_epochs * steps_per_epoch)
     
    cosine_epochs = max((num_epochs - warmup_epochs)/number_of_resets, 1)

    schedule_fns = [warmup_fn]
    boundaries = [warmup_epochs * steps_per_epoch]
    for j in range(number_of_resets):
        ...
        schedule_fns.append(cosine_fn)
        boundaries.append((warmup_epochs+(j+1)*cosine_epochs)*steps_per_epoch)

    schedule_fn = optax.join_schedules(
        schedules = schedule_fns,
